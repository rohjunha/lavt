...
def _compare_size(x, y):
    return x.size(-2) < y.size(-2) or x.size(-1) < y.size(-1)


def _fuse(x, y):
    if _compare_size(x, y):
        x = F.interpolate(input=x, size=(y.size(-2), y.size(-1)), mode='bilinear', align_corners=True)
    return x
